...
class BSTree {
    ...
    public:
		...
		void insert(T value, int criterio){
            contNodos++;
            insert(root, value, criterio);
        }
        ...
        */
        ...
        void insert(NodeBT<T>* &node, T value, int criterio){
            ...
            if (criterio == 1)
            {
                if(node == nullptr)
	    	    	...
	    	    	insert(node->left, value, criterio);
	    	    else if(value > node->data)
	    	    	insert(node->right, value, criterio);
